...
class ZodiacSwitch(app_manager.RyuApp):
	...
	def add_mpls_connection(self, dpid_src, dpid_dst, datapath, command, labeldfl, labelbu, onepath, path_list, src_MAC, dst_MAC, modify_dfl):
	...
	def _packet_in_handler(self, ev):
		...
		if ip4_pkt:
			...
			if (hash((scrDst_list[0], scrDst_list[1])) not in self.mpls_conn_list): 
				...
				self.add_mpls_connection(dpid_src, dpid_dst, datapath, command, labeldfl, labelbu, onepath, path_list, src_MAC, dst_MAC, modify_dfl)
				...
				for label in self.label_dfl_list:
					if (dpid in self.dfl_paths[str(label)]):
						if (dpid!=self.dfl_paths[str(label)][0] and dpid!=self.dfl_paths[str(label)][len(self.dfl_paths[str(label)])-1]):
							...
							self.add_mpls_connection(dpid_src, dpid_dst, datapath_src, command, labeldfl, labelbu, onepath, path_list, src, dst, modify_dfl)
